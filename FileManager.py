...
def load_single_users_from_xml(filename,user_id: int):
    tree = ET.parse(filename)
    root = tree.getroot()
    user : User
    for user_elem in root.findall("user"):
...
def load_addresses_from_xml(filename):
    tree = ET.parse(filename)
    root = tree.getroot()
    addresses = []
...
def saving_addresses_to_xml(addresses, filename):
    ...
    tree = ET.ElementTree(root)
    tree.write(filename, encoding="utf-8", xml_declaration=True)
    print(f"Адресса сохранены в файл {filename}")

def load_single_addresses_from_xml(filename,address_name : str):
    tree = ET.parse(filename)
    root = tree.getroot()
